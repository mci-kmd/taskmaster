...
vi.mock('./copilot-sdk-manager', () => ({
  CopilotSdkManager: class {
    ...
    loadSdk = async (): Promise<unknown> => ({
      ...
      module: {
        RuntimeConnection: {
          forStdio: (options: { path?: string; args?: readonly string[] } = {}) => ({
            kind: 'stdio',
            ...options
          })
        },
...
describe('Copilot session interactions', () => {
  it('passes global Copilot flags to the custom UI runtime', async () => {
    const service = setup()

    await service.start('thread')

    expect(harness.clientOptions?.connection).toEqual({
      kind: 'stdio',
      args: ['--yolo']
    })
  })
